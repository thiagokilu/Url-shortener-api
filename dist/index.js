...
app.post("/encurtar", async (req, res) => {
    try {
        ...
        await pool.query(`INSERT INTO urls (original, short, expires_at) VALUES ($1, $2, $3)`, [url, id, expires_at]);
...
app.get("/:id", async (req, res) => {
    try {
        ...
        const result = await pool.query(`SELECT original, expires_at FROM urls WHERE short = $1`, [id]);
        ...
        const updateResult = await pool.query(`UPDATE urls SET hits = hits + 1 WHERE short = $1 RETURNING hits`, [id]);
...
app.get("/stats/:id", async (req, res) => {
    try {
        ...
        const result = await pool.query(`SELECT original, hits, expires_at FROM urls WHERE short = $1`, [id]);
        if (result.rowCount === 0) {
